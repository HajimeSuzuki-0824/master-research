...
def load_audio(filepath):
    ...
    if y.ndim > 1:
        y = np.mean(y, axis=1)  # モノラルに変換
    return y, sr

def compute_spectral_crest(y, sr):
...
def softmax_crest(S_mod, beta=10):
...
def main():
    ...
    crest_a = compute_spectral_crest(y_a, sr_a)
    profiles_a = extract_profiles(crest_a, sr_a, onsets_a, offsets_a)
    ...
    out_path = f"sounds/outputs/{B_name}_crestTrans_sad.wav"
